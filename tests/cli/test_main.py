...
class TestPingCommand:
    ...
    def test_ping_success(self, mock_piglet_client: MagicMock) -> None:
        """ping should display success message and IP on valid credentials."""
        mock_piglet_client.ping.return_value = PingResponse(your_ip="203.0.113.42")
    ...
    def test_ping_auth_error(self, mock_piglet_client: MagicMock) -> None:
        """ping should show error message on authentication failure."""
        mock_piglet_client.ping.side_effect = AuthenticationError("Invalid credentials")
    ...
    def test_ping_passes_credentials(self, mock_piglet_client: MagicMock) -> None:
        """ping should pass API key and secret to client."""
        mock_piglet_client.ping.return_value = PingResponse(your_ip="1.2.3.4")

        with patch("oinker.cli._utils.Piglet", return_value=mock_piglet_client) as mock_piglet:
